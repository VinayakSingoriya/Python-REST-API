# Update entries to the database with the help of API
...
video_update_args.add_argument(
    "views", type=int, help="views of the video required")
video_update_args.add_argument(
...
class Video(Resource):
    ...
    def get(self, video_id):        # Used to get something
        ...
        if not result:
            abort(404, message="couldnt find video with that id...")
        return result
    ...
    def patch(self, video_id):
        ...
        result = VideoModel.query.filter_by(id=video_id).first()
        if not result:
            abort(404, message="video doesnot exist, cannnot update....")
        if args['name']:
